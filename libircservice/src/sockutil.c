/******************************************************************
 * ircservices lib is (C) CopyRight PTlink IRC Software 1999-2006 *
...
int sockprintf(int s, char *fmt, ...)
{
  ...
  for(cnt = 0; cnt <len; cnt+= wcnt) 
  {
    wcnt = write(s, (void *) (((char *) buf)+cnt),(unsigned) (len-cnt));
    if(wcnt < 0 && (errno != EAGAIN))
	return cnt;
  }
  return cnt;
}
