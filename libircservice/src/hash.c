 ...
 *  $Id: hash.c 33 2010-05-10 02:46:01Z openglx $
*/
...
#ifdef IRCD_PLEXUS
static struct HashEntry uidTable[U_MAX];
#endif
...
#ifdef IRCD_PLEXUS
size_t hash_get_uid_table_size(void)
{
  return sizeof(struct HashEntry) * U_MAX;
}
#endif
...
#ifdef IRCD_PLEXUS
unsigned int hash_uid(const char* name)
{
	return hash_nick_name(name);
}
#endif
...
/*
 * clear_uid_hash_table
 *
 * Nullify the hashtable and its contents so it is completely empty.
 */
#ifdef IRCD_PLEXUS
void clear_uid_hash_table()
{
  memset(uidTable, 0, sizeof(struct HashEntry) * U_MAX);
}
#endif
 ...
 */
#ifdef IRCD_PLEXUS
void add_to_uid_hash_table(const char* name, IRC_User* cptr)
{
  unsigned int hashv;

  hashv = hash_uid(name);
  cptr->uid_hnext = (IRC_User*) uidTable[hashv].list;
...
/*
 * del_from_uid_hash_table - remove a user/server from the uid
 * hash table
 */
#ifdef IRCD_PLEXUS
void del_from_uid_hash_table(const char* name, IRC_User* cptr)
{
  IRC_User* tmp;
  IRC_User* prev = NULL;
  unsigned int   hashv;
  ...
  hashv = hash_uid(name);
  tmp = (IRC_User*) uidTable[hashv].list;

  for ( ; tmp; tmp = tmp->uid_hnext)
    {
      if (tmp == cptr)
        {
          if (prev)
            prev->uid_hnext = tmp->uid_hnext;
          else
            uidTable[hashv].list = (void*) tmp->uid_hnext;
          tmp->uid_hnext = NULL;

          assert(uidTable[hashv].links > 0);
 ...
 */
#ifdef IRCD_PLEXUS
IRC_User* hash_find_uid(const char* name)
{
  IRC_User* tmp;
  unsigned int   hashv;
  assert(0 != name);
...
IRC_User* hash_find_server(const char* name)
{
  ...
  if (IsDigit(*name))
  {
	  ...
	  for ( ; tmp; tmp = tmp->uid_hnext)
	  {
	    if(tmp->sid && (irccmp(name, tmp->sid) == 0))
	      {
		return tmp;
	      }
	  }
