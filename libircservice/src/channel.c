...
IRC_ChanNode* add_user_to_chan(IRC_User* user, IRC_Chan *chan, int cumodes)
{
  ...
  /* -- Thales - when user is stealh, dont increases users count */ 
  if(!(irc_IsUMode(user, UMODE_STEALTH))) {
    chan->users_count++;
...
int irc_IsChanHOP(IRC_User* user, IRC_Chan* chan)
{
    IRC_ChanNode* cn;
    
    cn = chan->userlist;
    while(cn && cn->user != user)
	cn = cn->next;
	
    if(cn)
	return (cn->cumodes & CU_MODE_HOP);
    else
	return 0;
}
  ...
  while(node) /* search for the user to delete */
    {
      ...
      if(node->cumodes & CU_MODE_VOICE)
        cumodes[i++]='v';
      if(node->cumodes & CU_MODE_HOP)
        cumodes[i++]='h';        
      cumodes[i] = '\0';

      sendto_ircd(user->uid, "JOIN %d %s +", 
        time(NULL), node->chan->name);
      sendto_ircd(myserversid, "MODE %s +%s",
        node->chan->name, cmodes);
      while (i > 0)
      {
        sendto_ircd(myserversid, "MODE %s +%c %s",
          node->chan->name, cumodes[--i], user->nick);
      }
#else
      if(node->cumodes & CU_MODE_ADMIN)
        cumodes[i++]='.';
