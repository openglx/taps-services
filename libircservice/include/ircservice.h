 ...
 *  $Id: ircservice.h 33 2010-05-10 02:46:01Z openglx $
*/
...
/** my odd tries to include IRCD PLEXUS protocol support -- openglx */
#ifdef IRCD_PLEXUS
/* these came form hybrid7 source */
...
/*  char vhost[HOSTLEN + 1]; */
  ...
  /** uid and sid are to make taps ircd plexus compatible */
#ifdef IRCD_PLEXUS
  char uid[IDLEN + 1];
  char sid[IRC_MAXSID + 1];
  struct IRC_User_s* uid_hnext;	/* for user hash */
#endif
...
struct IrcMsg_s
{
  ...
  MsgHandler func;
  IrcMsg* next;
  int use_match;	/** Should we use match ? */
};

/* external variables */
#ifdef IRCD_PLEXUS
extern char myserversid[IRC_MAXSID];
#endif

/** ircd type, we only support PTlink6 for now -- << not anymore (openglx) */
 ...
 */
#ifdef IRCD_PLEXUS
extern int irc_Init(int stype, char *sname, char *sinfo, char *ssid, FILE *logfd);
#else
extern int irc_Init(int stype, char *sname, char *sinfo, FILE *logfd);
#endif

/**
 * Initializes curred all time record for user peak.
 * Params:\n
 *  peak = all time user peak, for recordstats module access to SQL
 *
 */
extern int irc_SetPeak (unsigned int peak);
...
#define irc_IsCNOwner(x)       ((x)->cumodes & CU_MODE_OWNER)
...
#define ET_PEAKUSER     12 /* Issue 27, PeakUser -- Thales */
...
#else
#define CMODE_z         0x00002000  /* channel is registered */
#endif
