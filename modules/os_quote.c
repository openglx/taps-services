 ...
 *  $Id: os_quote.c 33 2010-05-10 02:46:01Z openglx $
*/
...
{"os_quote", "2.1",  "operserv random quots module" };

/* Change Log
  2.1 - #14: Quote EDIT command add
...
void os_quote(IRC_User *s, IRC_User *u)
{
  ...
  else if(strcasecmp(cmd, "EDIT") == 0)
  {
    char *sid = strtok(NULL, " ");
    char *quote = strtok(NULL, "");
    if(IsNull(sid) || IsNull(quote))
	send_lang(u, s, QUOTE_EDIT_INVALID_SYNTAX);
    else
    {
	if(sql_singlequery("SELECT quote from os_quote where id=%d", atoi(sid)) == 0)
	    send_lang(u, s, QUOTE_X_NOT_FOUND, atoi(sid));
	else
	{
	    int r=sql_execute("UPDATE os_quote SET quote='%s' WHERE id=%d", quote, atoi(sid));
	    if(r == 1)
		send_lang(u, s, QUOTE_X_UPDATED, atoi(sid));
	    else
    
		send_lang(u, s, QUOTE_UPDATE_FAILED); /* We hopefuly never get here ;) */
	}
    }
  }    
