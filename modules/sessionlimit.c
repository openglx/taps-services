...
{"sessionlimit", "1.3",  "session limit module" };

/* Change Log 
  1.3 - Display OWNER information on LIST and SHOW.
  ...
  else
  {
    ...
    for(hashv = 0; hashv < HOST_HSIZE; ++hashv)
    {
      ...
      while(hr)
      {
        ...
        {
          if(hr->limit_hits)
            send_lang(u, s, OS_SESSION_LIST_ITEM_host_count_limit_hits, 
              hr->host, hr->count, hr->limit, hr->limit_hits);
          else
            send_lang(u, s, OS_SESSION_LIST_ITEM_host_count_limit, 
  ...
  else
  {
    ...
    while(list_u)
    {
      if(strcmp(hr->host, list_u->realhost) == 0) /* we have a match*/
      	send_lang(u, s, OS_SESSION_VIEW_ITEM_nick_mask_info,
...
static void os_exception_list(IRC_User *s, IRC_User *u)
{
  ...
  /* until exception list owner allows dropped nicks, we need to take a few cares */
  int has_valid_nick = 0;
  int row_snid = -1;
  ...
  else
  {
	...
	if(privilege || (u->snid == row_snid))
	{
         HostRecord* hr = find_session(row[0]);

	if(row_snid >= 0)
	{
		if(sql_singlequery("SELECT nick FROM nickserv WHERE snid=%d", row_snid)==0)
			has_valid_nick = 0;
		else
			has_valid_nick = 1;
	}
        ...
        if(hr)
    	    send_lang(u, s, OS_EXCEPTION_LIST_host_slimit_count_owner_reason,
    	    row[0], row[5], hr->count, has_valid_nick ? sql_field(0) : "(unknown on db)", row[6]);
        else
    	    send_lang(u, s, OS_EXCEPTION_LIST_host_slimit_owner_reason,
    	    row[0], row[5], has_valid_nick ? sql_field(0) : "(unknown on db)", row[6]);
	}
