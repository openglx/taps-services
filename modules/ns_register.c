...
{"ns_register", "2.4","nickserv register command" };
...
DBCONF_REQUIRES
  ...
  DBCONF_GET("nickserv", UsersEmailDomain)
DBCONF_END
...
void ns_register(IRC_User *s, IRC_User *u)
{
  ...
  /* check requirements */
  ...
  else /* execute operation */
    {
      ...
      if(UsersEmailDomain)
      {
        char network_email[128];
        snprintf(network_email, sizeof(network_email), "%s@%s",
          irc_lower_nick(u->nick), UsersEmailDomain);
        sqlb_add_str("network_email", network_email);
      }
