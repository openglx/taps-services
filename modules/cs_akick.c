 ...
 *  $Id: cs_akick.c 33 2010-05-10 02:46:01Z openglx $
*/
...
static int DefaultAkickTime;

DBCONF_PROVIDES
  DBCONF_INT(MaxAkicksPerChan, "20", "Max. number of of akicks per channel")
  DBCONF_INT(DefaultAkickTime, "5", "Default amount of days for an akick to expire")
DBCONF_END
/* Remote config */
...
void cs_akick(IRC_User *s, IRC_User *u)
{
  ...
  else if(strcasecmp(cmd, "ADD") == 0) /* Add akick */
  {
    ...
    if(reason)
      ...
      duration = DefaultAkickTime*24*3600; /* default akick time is one week */
