...
SVS_Module mod_info = 
  {"nickserv", "7.4", "nickserv core module" };
/* Change Log
  7.4
    TAPS first revision.
    Issue #31 - CTCP PING from user to services isn't correctly interpreted
  7.3
    #89: nickserv.UsersEmailDomain for networks providing email addresses
    #76, multichat messages support
...
/* Issue 031 reported by openglx
 * Properly respond to CTCP PING
 * --after
 */
void ns_ctcp_ping(IRC_User *s, IRC_User *t);
...
static char* UsersEmailDomain = NULL;
...
DBCONF_PROVIDES
  ...
  DBCONF_WORD_OPT(UsersEmailDomain, NULL,
    "If defined will be used to fill the network_email field")
DBCONF_END
...
int mod_load(void)
{
 ...
 irc_AddUMsgEvent(nsu.u, "\1PING", ns_ctcp_ping);
...
void ev_ns_new_user(IRC_User* u, void *null)
{
  ...
  { /* user already identified for this nick */
