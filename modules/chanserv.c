...
SVS_Module mod_info =
{"chanserv", "5.2", "chanserv core module" };

#define DB_VERSION	4

/* ChangeLog
  5.2 - TAPS #1: allow event for chanmode -o, reop localusers if they are -o
        #31 - CTCP PING from user to services isn't correctly interpreted
...
/* commands */
void cs_ctcp_ping(IRC_User *s, IRC_User *t);
...
int mod_load(void)
{
  ...
  if(LogChan)
  {
    ...
    /* set chanserv user to be LogChan local_user -- openglx for issue #16 */
    chan->local_user = csu.u;
  }
        
  irc_AddUMsgEvent(csu.u, "\1PING", cs_ctcp_ping);
 ...
 * ev_cs_deop
 * 
 * See TAPS bug #1 to understand this.
 *  -- openglx
 */
void ev_cs_deop(IRC_Chan *chan, IRC_User *user)
{
 ...
 /* OLD? send_lang(user, csu.u, CHAN_DEOP, user->nick, chan->name); */

 /* don't allow someone deop one of our local users */
 if (irc_IsLocalUser(user))
 {
  /*
   * I'd like to use irc_ChanUMode but on my tests it didn't work... 
   * and on modules/botserv.c we have this piece of code.
   */
  irc_ChanMode(csu.u, chan, "+o %s", user->nick);
 }

 /* call the event */
 if (cr)
 {
  /* untested, but should work if careful enough */
  mod_do_event(e_regchan_deop, cr, user);
 }
