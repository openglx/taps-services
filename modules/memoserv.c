...
SVS_Module mod_info = {"memoserv", "5.2", "memoserv core module" };

#define DB_VERSION      5

/* ChangeLog
  5.2 - TAPS first revision
        #31 - CTCP PING from user to services isn't correctly interpreted
  5.1 - # Add the notify part and enable notify for all nicknames that are registered
...
DBCONF_PROVIDES
  ...
  DBCONF_STR(DefaultOptions, "notify", "Default memoserv options")
DBCONF_END
...
/* commands */
void ms_ctcp_ping(IRC_User *s, IRC_User *t);
...
int mod_load(void)
{
  ...
  irc_AddUMsgEvent(msu.u, "\1PING", ms_ctcp_ping);
